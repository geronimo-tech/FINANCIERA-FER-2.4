...
import { 
  ...
  cambiarFrecuenciaPago,
} from '@/lib/prestamos-store'
import type { Prestamo, PagoMensual, FrecuenciaPago } from '@/lib/types'
...
export function PrestamoDetail({ id }: PrestamoDetailProps) {
  ...
  const handleEditPrestamo = useCallback(() => {
    ...
    // Primero actualizar datos basicos (nombre, telefono, domicilio, capital, tasa, fecha)
    const { frecuenciaPago: _freq, montoCuota: _cuota, ...basicUpdates } = editForm
    let updated = updatePrestamo(id, basicUpdates)
    
    // Si cambio la frecuencia, regenerar pagos
    if (updated && editForm.frecuenciaPago !== prestamo.frecuenciaPago) {
      updated = cambiarFrecuenciaPago(
        id, 
        editForm.frecuenciaPago,
        editForm.frecuenciaPago === 'semanal' ? editForm.montoCuota : undefined
      )
    }
                ...
                <DialogContent>
                  ...
                  <div className="space-y-4 py-4 max-h-[60vh] overflow-y-auto">
                    ...
                    <div className="border-t pt-4 space-y-4">
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="edit-frecuencia">Tipo de Prestamo (Frecuencia)</Label>
                        <Select
                          value={editForm.frecuenciaPago}
                          onValueChange={(val: FrecuenciaPago) => setEditForm(prev => ({ ...prev, frecuenciaPago: val }))}
                        >
                          <SelectTrigger>
                        ...
                        {editForm.frecuenciaPago !== prestamo?.frecuenciaPago && (
                          <p className="text-xs text-orange-600 font-medium">
                            Cambiar la frecuencia regenerara todos los pagos pendientes.
                          </p>
                        )}
                      </div>
                      {editForm.frecuenciaPago === 'semanal' && (
                        <div className="space-y-2">
                          <Label htmlFor="edit-cuota">Cuota Semanal</Label>
                          <Input
                            id="edit-cuota"
                            type="number"
                            min="0"
                            value={editForm.montoCuota || ''}
                            onChange={(e) => setEditForm(prev => ({ ...prev, montoCuota: parseFloat(e.target.value) || 0 }))}
                          />
                          <p className="text-xs text-muted-foreground">
                            Monto fijo que se cobra cada semana.
                          </p>
                        </div>
                      )}
                      {editForm.frecuenciaPago === 'quincenal' && (
                        <div className="rounded-lg border bg-muted/50 p-3">
                          <p className="text-xs text-muted-foreground">
                            Cuota quincenal (interes x2): <span className="font-bold text-foreground">{formatCurrency(editForm.montoCapital * (editForm.tasaInteres / 100) * 2)}</span>
                          </p>
                        </div>
                      )}
                      {editForm.frecuenciaPago === 'mensual' && (
                        <div className="rounded-lg border bg-muted/50 p-3">
                          <p className="text-xs text-muted-foreground">
                            Interes mensual: <span className="font-bold text-foreground">{formatCurrency(editForm.montoCapital * (editForm.tasaInteres / 100))}</span>
                          </p>
                        </div>
                      )}
                      <div className="space-y-2">
